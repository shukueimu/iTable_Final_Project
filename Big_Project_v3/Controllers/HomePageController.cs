...
namespace Big_Project_v3.Controllers
{
	public class HomePageController : Controller
	{
		...
		[HttpGet]
		public IActionResult Index(string district = "西屯區")
		{
			...
			var HPModel = _context.Restaurants
				...
				.Select(x => new HomePageItems
				{
					...
					ImageUrl = "/img/restaurant1.jpg",    //x.photo != null ? x.photo.PhotoURL : "/img/default.jpg"
					CustomerName = x.user != null ? x.user.Name : "Anonymous",
                    PriceRange = !string.IsNullOrEmpty(x.restaurant.PriceRange) && x.restaurant.PriceRange.Contains("$")
					     ? Convert.ToInt32(x.restaurant.PriceRange.Substring(x.restaurant.PriceRange.IndexOf('$') + 1).Replace(",", ""))
    :					 0, // 如果 PriceRange 為 null，則設置為默認值 0,
                    Url = "#"
				})
			...
			var restaurants = HPModel.Where(vm => vm.RestaurantRating > 4.5).Select(vm => new HomePageItems
			{
            ...
            var nearbyRestaurants = HPModel.Where(vm => vm.RestaurantAddress.Contains(district)).Select(vm => new HomePageItems
			{
				RestaurantID = vm.RestaurantID,
				RestaurantName = vm.RestaurantName,
				RestaurantAddress = vm.RestaurantAddress,
			...
			var activeRestaurants = HPActiveModel.Where(vm => !string.IsNullOrWhiteSpace(vm.Title)).Select(vm => new HomePageItems
			{
				RestaurantID = vm.RestaurantID,
				RestaurantName = vm.RestaurantName,
				RestaurantAddress = vm.RestaurantAddress,
